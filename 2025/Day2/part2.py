...
def is_number_repeated(number_str: str) -> bool:
    half_length = len(number_str) // 2
    for i in range(half_length):
        substr = number_str[0: i + 1]
        substr_len = len(substr)
        scan_idx = i + 1
        while scan_idx + substr_len <= len(number_str):
            if number_str[scan_idx: scan_idx + substr_len] != substr:
                break
            scan_idx += substr_len
        if scan_idx == len(number_str):
            return True
    return False

def brute_force_part2(first: str, second: str) -> None:
    first_start, first_end = int(first), int(second)
    matches = []
    for number in range(first_start, first_end + 1):
        number_str = str(number)
        if is_number_repeated(number_str):
...
def part2(input_data):
    ...
    for range_raw in ranges:
        ...
        summed_matches += sum(brute_force_part2(first, second))
